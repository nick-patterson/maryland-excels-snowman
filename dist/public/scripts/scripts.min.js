...
var snowman={toModify:"",snowmanElements:{hat:[],face:[],head:[],torso:[],arms:[],base:[],scene:[]},current:{hat:0,face:0,head:0,torso:0,arms:0,base:0,scene:0},init:function(){snowman.toModify=$(".build-toolbar-button-active").data("target"),snowman.draw()},draw:function(){function e(e,n){return"prev"==n?0==h.current[e]?{current:h.current[e],next:h.current[h.snowmanElements[e].length]}:{current:h.current[e],next:h.current[e]-1}:h.current[e]==h.current[h.snowmanElements[e].length]?{current:h.current[e],next:0}:{current:h.current[e],next:h.current[e]+1}}function n(){for(i=0;i<w.length;i++){var e=window.innerWidth,n=window.innerHeight;h.snowmanElements.scene[i]=new fabric.Image(w[i]);var t=h.snowmanElements.scene[i].getWidth(),a=h.snowmanElements.scene[i].getHeight();if(a>t)var o=n/a*1.2;else var o=e/t*1.2;h.snowmanElements.scene[i].selectable=!1,h.snowmanElements.scene[i].setScaleY(o),h.snowmanElements.scene[i].setScaleX(o),h.snowmanElements.scene[i].setOpacity(0),h.snowmanElements.scene[i].OriginY="center",h.snowmanElements.scene[i].OriginX="center",h.snowmanElements.scene[i].left=(e-t*o)/2,h.snowmanElements.scene[i].top=(n-a*o)/2,h.snowmanElements.scene[0].setOpacity(1),r.add(h.snowmanElements.scene[0])}}function t(e,n){r.add(h.snowmanElements.scene[n]),TweenMax.to(h.snowmanElements.scene[e],.25,{opacity:0,onComplete:function(){r.remove(h.snowmanElements.scene[e])}}),TweenMax.to(h.snowmanElements.scene[n],.25,{opacity:1})}function a(e){return r.width*(e/100)}function o(e){return r.height*(e/100)}function s(){r.renderAll(),window.requestAnimationFrame(s)}var r=new fabric.Canvas("snowman-canvas");r.setDimensions({width:window.innerWidth,height:window.innerHeight});var c=document.getElementById("bg-1"),l=document.getElementById("bg-2"),m=document.getElementById("bg-3"),w=[c,l,m];n();var d=new fabric.Rect({width:200,height:200,top:200,fill:"green",originX:"center",originY:"center"}),u=new fabric.Rect({width:100,height:100,top:50,fill:"blue",originX:"center",originY:"center"}),b=new fabric.Rect({width:50,height:50,top:-25,fill:"teal",originX:"center",originY:"center"}),h=new fabric.Group([d,u,b]);h.originX="center",h.originY="bottom",h.selectable=!1,h.left=a(50),h.top=o(70),r.add(h),$(".build-button").click(function(n){if($(this).is("#build-button-prev"))switch(h.toModify){case"hat":break;case"face":break;case"head":break;case"torso":break;case"arms":break;case"base":break;case"scene":var i=e("scene","prev"),a=i.current,o=i.next;t(a,o)}else switch(h.toModify){case"hat":break;case"face":break;case"head":break;case"torso":break;case"arms":break;case"base":break;case"scene":}}),$(window).click(function(e){}),$(window).resize(function(e){r.setDimensions({width:window.innerWidth,height:window.innerHeight}),h.left=a(50),h.top=o(70)}),s()}},toolbar={init:function(){$(".build-toolbar-button").click(function(){$(".build-toolbar-button").not($(this)).removeClass("build-toolbar-button-active"),$(this).addClass("build-toolbar-button-active"),snowman.toModify=$(this).data("target")})}};$(window).load(function(){snowman.init(),toolbar.init()});
